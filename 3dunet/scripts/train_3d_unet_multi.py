...
settings_path = Path(dir_path, 'settings',
                     '200724_3dUnet_sand_brine_meth_multilabel_3cGDL.yaml')
...
def prepare_batch(batch, device):
    ...
    channel_3 = torch.zeros_like(targets)
    channel_1[targets == 0] = 1
    channel_2[targets == 1] = 1
    channel_3[targets == 2] = 1
    targets = torch.stack((channel_1, channel_2, channel_3), 1).to(device, dtype=torch.uint8)
